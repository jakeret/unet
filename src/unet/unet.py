...
def conv_block(x, layer_idx, filters_root, kernel_size, dropout_rate):
...
def upconv_block(x, layer_idx, filters_root, kernel_size, pool_size):
...
def crop_concat_block(x, down_layer):
    ...
    with tf.name_scope("contracting"):
        for layer_idx in range(0, layer_depth - 1):
            with tf.name_scope(f"contracting_{layer_idx}"):
                x = conv_block(x, layer_idx, filters_root, kernel_size, dropout_rate)
                down_layers[layer_idx] = x
                x = max_pool_layer(x, pool_size)

    with tf.name_scope("bottom"):
        x = conv_block(x, layer_idx + 1, filters_root, kernel_size, dropout_rate)

    with tf.name_scope("expanding"):
        for layer_idx in range(layer_depth - 2, -1, -1):
            with tf.name_scope(f"expanding_{layer_idx}"):
                x = upconv_block(x, layer_idx, filters_root, kernel_size, pool_size)
